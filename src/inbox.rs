...
fn get_command_request(body: &str) -> Result<CommandRequest, (StatusCode, String)> {
    let clean_body = decode_quoted_printable(body).map_err(|e| {
        let err = format!("Failed to decode quoted-printable: {}", e);
        error!("{}", err);
    ...
    let re =
        Regex::new(r#"<div[^>]*id="[^"]*relayer-data[^"]*"[^>]*>(.*?)</div>"#).map_err(|e| {
            let err = format!("Failed to compile relayer data regex: {}", e);
            error!("{}", err);
    ...
    let relayer_data = re
        ...
        .ok_or_else(|| {
            let err = "Failed to extract relayer data".to_string();
            error!("{}", err);
            (StatusCode::BAD_REQUEST, err)
        })?
        .as_str();
    ...
    let anchor_re = Regex::new(r#"<a[^>]*>([^<]+)</a>"#).map_err(|e| {
        let err = format!("Failed to compile anchor regex: {}", e);
        error!("{}", err);
        (StatusCode::BAD_REQUEST, err)
    })?;
    ...
    let command_request: CommandRequest =
        serde_json::from_str(&decoded_relayer_data).map_err(|e| {
            let err = format!("Failed to parse relayer data: {}", e);
            error!("{}", err);
            (StatusCode::BAD_REQUEST, err)
        })?;
...
pub async fn inbox_handler(
    ...
    let command_request = get_command_request(&body).map_err(|e| {
        error!("Failed to get command request: {}", e.1);
        (StatusCode::BAD_REQUEST, e.1)
    })?;
    info!("{:?}", command_request);

    let proof = generate_proof(&body, &state.prover).await.map_err(|e| {
